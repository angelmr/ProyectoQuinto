...
public class FUsuario implements Serializable {
    ...
    public static ArrayList<Usuario> llenarUsuarios(ConjuntoResultado rs) throws Exception {
        ...
        try {
            while (rs.next()) {
                usuario = new Usuario(rs.getLong("pcodigo"),rs.getString("papellidos"),rs.getString("pnombres"),
                        rs.getString("pidentificacion"),rs.getString("pnombre_corto"),rs.getString("pclave"),
                rs.getString("pclave_pregunta"),rs.getString("pclave_respuesta"),rs.getString("pmail"),rs.getInt("pestado"),
                rs.getTimeStamp("pprimer_acceso"),rs.getTimeStamp("pultimo_acceso"),rs.getString("pultima_ip"),rs.getTimeStamp("pfecha_modificacion"),
                rs.getString("pcodigo_salt"),rs.getString("pruta_firma"));
                lst.add(usuario);
            }
        } catch (Exception e) {
            lst.clear();
